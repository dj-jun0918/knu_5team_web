  ...
  cartStorage.forEach((item) => {
    ...
    productDiv.innerHTML = `
          <div>
            ...
            <img src="${item.imgUrl}" alt="${item.title}" style="width: 100px;"> <!-- 상품 이미지 -->
            <p>가격: ${item.price} 원</p> <!-- 상품 가격 -->
            <p>수량: ${item.orderCount}</p> <!-- 구매 수량 -->
            <button id = "delete_product" onclick="removeFromCart(${item.id})">상품 삭제</button>
          <div>
        `;
...
const productSum = () => {
  ...
  document.getElementById("productSum").innerText = `상품총합: ${totalSum}`;
};
